...
{
    [Filters.AdminRole]
    public class AccessoryImportController : CommonController
    {
        void BindList(int pageNo, List<AccessoryImport> cate)
        {
            if (cate != null)
            {
                ViewBag.AccessoryImports = cate.OrderByDescending(p => p.ImportOn).Skip((pageNo - 1) * PagingHelper.PageSize()).Take(PagingHelper.PageSize());
                ViewBag.TotalRecords = cate.Count();
            }
            else
            {
                ViewBag.AccessoryImports = db.AccessoryImports.OrderByDescending(p => p.ImportOn).Skip((pageNo - 1) * PagingHelper.PageSize()).Take(PagingHelper.PageSize());
        ...
        public ActionResult Index(int pageNo = 1, int accesoryid =0, int id = 0, string searchString = "")
        {
            ...
            var cate = from s in db.AccessoryImports where s.AccessoryId == accesoryid
                       select s ;
            ...
            var model = new AccessoryImport();
            if (id > 0) model = db.AccessoryImports.Find(id);
            model.AccessoryId = accesoryid;
            //ViewBag.Employees = new SelectList(db.Employees.ToList(), "Id", "Name", model.ImportToEmployeeId);
        ...
        // GET: Category/Create
        public ActionResult Create(AccessoryImport model, int pageNo = 1)
        {
            ...
            //    TempData["Error"] = Language.AccessoryImport.AccessoryImport.OutOfAmountError;
            //}
            //if (model.ImportToEmployeeId == null)
            //{
            //    ModelState.AddModelError("Invalid Amount", "Invalid Amount");
            //    TempData["Error1"] = Language.AccessoryImport.AccessoryImport.EmpError;
            //}
        ...
        {
            var accessImport = db.AccessoryImports.Find(id);
            accessImport.Cancel = true;
            db.Entry(accessImport).State = EntityState.Modified;

            var accessory = db.Accessories.Find(accessImport.AccessoryId);
            accessory.Amount = accessory.Amount - accessImport.Amount;
            db.Entry(accessory).State = EntityState.Modified;
            db.SaveChanges();
            return RedirectToAction("Index", new { PageNo = 1, accesoryid = accessImport.AccessoryId });
        }
        ...
        public ActionResult Delete(int id, int pageNo = 1)
        {
            if (id > 0)
            {
                db.Entry(db.AccessoryImports.Find(id)).State = EntityState.Deleted;
                db.SaveChanges();
            }
        ...
        public ActionResult Import(string searchString = "")
        {
